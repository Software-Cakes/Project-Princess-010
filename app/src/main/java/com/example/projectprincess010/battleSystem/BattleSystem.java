...
public class BattleSystem {
    ...
    public ThreatAttackResult executeThreatAttack() {
        ...
        String fullMessage = roleMessage + " " + target.getName() + " takes " + damage +
                " damage. (Threat HP: " + threat.getCurrentHP() + "/" + threat.getMaxHP() + "). Choose your next move to neutralize the threat!";
        ...
        if (target.isIncapacitated() && !target.isRevived()) {
            if (characterA.isIncapacitated() && characterB.isIncapacitated()) {
                ...
                fullMessage = target.getName() + " has succumbed to the damages and requires immediate medical attention. " +
                        remaining.getName() + " must now complete the mission alone. Good luck! (Threat HP: " + 
                        threat.getCurrentHP() + "/" + threat.getMaxHP() + ")";
                battleLog.add(fullMessage);
    ...
    public AbilityResult executeCrewAction(String abilityName) {
        ...
        if (!activeCharacter.isAbilityAvailable(abilityName)) {
            ...
            battleMessageInLog(lockedMessage);
        ...
        if (result.success) {
            // Always update damage dealt, even if 0, to keep multipliers correct
            ...
            if (!threat.isDefeated()) {
                actionMessage += " (Threat HP: " + threat.getCurrentHP() + "/" + threat.getMaxHP() + ")";
            }
            battleLog.add(actionMessage);
            
            if (!result.message.isEmpty() && !result.message.equals(actionMessage)) {
                if (!threat.isDefeated() && !result.message.contains("Threat HP")) {
                    result.message += " (Threat HP: " + threat.getCurrentHP() + "/" + threat.getMaxHP() + ")";
                }
    ...
    private void battleMessageInLog(String message) {
        if (!battleLog.contains(message)) {
