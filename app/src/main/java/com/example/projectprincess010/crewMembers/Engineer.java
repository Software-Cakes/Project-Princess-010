...
public class Engineer extends CrewMember {
    ...
    private AbilityResult useEMPulse(Threat threat) {
        ...
        int damageToThreat = 60; // Added base damage
        threat.addDamageReductionDebuff(15, 2);
        usedEMPulse = true;

        String message = name + " fires EMP Pulse, dealing " + damageToThreat + " damage and reducing threat damage by 15% for 2 turns!";
        return new AbilityResult(message, damageToThreat, 0, 0, true);
    }
    ...
    private AbilityResult useSystemReroute(Threat threat) {
        ...
        if (!debuffs.isEmpty()) {
            int damageToThreat = 40; // Added base damage
            threat.addDamageReductionDebuff(20, 1);
            debuffs.clear();
            String message = name + " reroutes system, dealing " + damageToThreat + " damage and reducing threat damage by 20%!";
            return new AbilityResult(message, damageToThreat, 0, 0, true);
        } else {
